...
def ajouter():
    ...
    tree.insert("", END, text="", values=(nom, prix, qte_stock,
                seuil_alerte, date_premiere_entree, date_derniere_sortie, id_admin))
    # Store the inputs in the database table
    pro = produit(nom_input.get(), prix_input.get(), qte_input.get(), seuil_input.get(
    ), date_derniere_entree_input.get(), date_derniere_sortie_input.get(), id_admin)
...
def supprimer():
    ...
    if not selected_item:
        ...
        return
    admin_id = tree.item(selected_item)['values'][-1]
    if(logged_in_admin == admin_id):
        ...
        c.supprimerProduit(nom_input.get())
    else:
...
def afficher():
    try:
        ...
        for produit in produits:
            ...
            id_admin = produit[7]
            tree.insert("", END, values=(nom, prix, qte_stock, seuil_alerte,
                        date_premiere_entree, date_derniere_sortie, id_admin))
...
def rechercher():
    search_term = entry.get().lower()  # Récupérer le terme de recherche saisi
    # Effacer les éléments existants dans le Treeview
    tree.delete(*tree.get_children())

    # Parcourir les produits de la base de données et afficher les correspondances
    produits = c.rechercherProduit(search_term)
    for produit in produits:
        nom = produit[1]
        prix = produit[2]
        qte_stock = produit[3]
...
heading = Label(frame, text="Infor des produits", font=(
    "Microsoft Yahei UI Light", 18, "bold"), fg="black")
heading.grid(row=0, column=0)

# nom du produit
nom_label = Label(frame, text="nom du produit", font=(
    "Microsoft Yahei UI Light", 10, "bold"), pady=10)
nom_label.grid(row=1, column=0, sticky="w", padx=10)

nom_input = Entry(frame, width=30, font=(
    "Microsoft Yahei UI Light", 10, "bold"))
nom_input.grid(row=2, column=0)

# prix
prix_label = Label(frame, text="prix", font=(
    "Microsoft Yahei UI Light", 10, "bold"), pady=10)
prix_label.grid(row=3, column=0, sticky="w", padx=10)

prix_input = Entry(frame, width=30, font=(
    "Microsoft Yahei UI Light", 10, "bold"))
prix_input.grid(row=4, column=0)

# qte_stock
qte_label = Label(frame, text="quantité en stock", font=(
    "Microsoft Yahei UI Light", 10, "bold"), pady=10)
qte_label.grid(row=5, column=0, sticky="w", padx=10)

qte_input = Entry(frame, width=30, font=(
    "Microsoft Yahei UI Light", 10, "bold"))
qte_input.grid(row=6, column=0)

# seuil alerte
seuil_label = Label(frame, text="seuil d'alerte", font=(
    "Microsoft Yahei UI Light", 10, "bold"), pady=10)
seuil_label.grid(row=7, column=0, sticky="w", padx=10)

seuil_input = Entry(frame, width=30, font=(
    "Microsoft Yahei UI Light", 10, "bold"))
seuil_input.grid(row=8, column=0)

# date_derniere_entree
date_derniere_entree_label = Label(frame, text="date de la dernière entrée", font=(
    "Microsoft Yahei UI Light", 10, "bold"), pady=10)
date_derniere_entree_label.grid(row=9, column=0, sticky="w", padx=10)

date_derniere_entree_input = Entry(
    frame, width=30, font=("Microsoft Yahei UI Light", 10, "bold"))
date_derniere_entree_input.grid(row=10, column=0)

# date_derniere_sortie
date_derniere_sortie_label = Label(frame, text="date de la dernière sortie", font=(
    "Microsoft Yahei UI Light", 10, "bold"), pady=10)
date_derniere_sortie_label.grid(row=11, column=0, sticky="w", padx=10)

date_derniere_sortie_input = Entry(
    frame, width=30, font=("Microsoft Yahei UI Light", 10, "bold"))
date_derniere_sortie_input.grid(row=12, column=0)

# add btn
ajouter_btn = Button(frame, text="Ajouter", font=(
    "Open Sans", 16, "bold"), command=ajouter)
...
# delete btn
delete_btn = Button(frame, text="Supprimer", font=(
    "Open Sans", 16, "bold"), command=supprimer)
delete_btn.grid(row=10, column=1)

# rechercher btn
rechercher_btn = Button(frame, text="Rechercher", font=(
    "Open Sans", 16, "bold"), command=rechercher)
rechercher_btn.grid(row=11, column=1)
# Créer la zone de recherche (Entry widget)
entry = Entry(frame, width=30, font=("Microsoft Yahei UI Light", 10, "bold"))
entry.grid(row=12, column=1)

# Afficher btn
afficher_btn = Button(frame, text="Afficer", font=(
    "Open Sans", 16, "bold"), command=afficher)
afficher_btn.grid(row=13, column=1)

# Create a treeview widget:
tree = ttk.Treeview(frame)
tree["columns"] = ("nom", "prix", "qte_stock", "seuil_alerte",
                   "date_premiere_entree", "date_derniere_sortie",  "id_admin")
